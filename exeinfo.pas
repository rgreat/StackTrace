...
function FindExeSection(var e:TExeFile;const secname:shortstring;var secofs,seclen:integer):boolean;
...
{$i ostypes.inc}
  ...
  function get_next_image_info(team: team_id; var cookie:integer; var info:image_info; size: size_t) : status_t;cdecl; external 'root' name '_get_next_image_info';
...
function getByte(var f:file):byte;
  ...
  procedure Skip (var f:file; bytes : integer);
  var i : integer;
  begin
  ...
  function getint32 (var f:file): integer;
  begin
...
function FindSectionNetwareNLM(var e:TExeFile;const asecname:shortstring;var secofs,seclen:integer):boolean;
...
function FindSectionCoff(var e:TExeFile;const asecname:shortstring;var secofs,seclen:integer):boolean;
var
  i : integer;
  sechdr     : tcoffsechdr;
  secname    : shortstring;
...
type
  ...
  TAoutHeader = packed record
     ...
     SymbSize: integer;
     EntryPoint: integer;
     TextRelocSize: integer;
     DataRelocSize: integer;
  end;
...
function FindSectionEMXaout (var E: TExeFile; const ASecName: shortstring;
                                         var SecOfs, SecLen: integer): boolean;
begin
...
type
  telfheader=packed record
      ...
      padding           :  packed array[0..6] of Byte;
      e_type            : word;
      e_machine         : word;
      e_version         : Cardinal;
...
begin
  ...
  { Try, avoided in order to remove exception installation }
    var s := PAnsiChar(AnsiString(envp^));
    pc:= @s;
    phdr_count:=-1;
    phdr_size:=0;
    phdr:=nil;
    ...
    while assigned(pat_hdr) do
      begin
        ...
        if pat_hdr^.typ = AT_EXE_FN then
          filename:=ansistring(pchar(pat_hdr^.value));
        inc (PByte(pat_hdr),sizeof(AT_HDR));
      end;
...
function FindSectionElf(var e:TExeFile;const asecname:shortstring;var secofs,seclen:integer):boolean;
var
  elfsec     : telfsechdr;
  secname    : ansistring;
  secnamebuf : array[0..255] of ansichar;
...
*)
  ...
  tmach_fat_archs = array[1..high(integer) div sizeof(tmach_header)] of tmach_fat_arch;
...
function FindSectionMachO(var e:TExeFile;const asecname:shortstring;var secofs,seclen:integer):boolean;
var
   i, j: cuint32;
   cmd: pmach_load_command;
...
type
  ...
  TFindSectionProc=function(var e:TExeFile;const asecname:shortstring;var secofs,seclen:integer):boolean;
...
begin
  ...
  E.ImgOffset := 0;
  if Assigned(ExeProcs.OpenProc) then
...
begin
  ...
  if not e.isopen then
    exit;
  if Assigned(ExeProcs.FindProc) then
...
begin
  ...
  CheckDbgFile:=(dbgcrc=c);
    ...
    function align(i,a:integer):integer;{$ifdef USEINLINE}inline;{$endif}
    {
...
begin
  ...
  { executable dir }
  i:=length(e.filename);
  while (i>0) and not CharInSet(e.filename[i], AllowDirectorySeparators) do
    dec(i);
  if i>0 then
    begin
